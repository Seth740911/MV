...
function playVid(url) {
    ...
    v.volume = 0.3;
    v.muted = false;
    v.playbackRate = _spdList[_spdIdx]; // 恢复上次速度
    ...
    // VIA浏览器接管视频全屏时，返回后自动退出播放器，不留中间页
    v.addEventListener('webkitendfullscreen', function() {
        stopVid();
    });
    // 兜底：视频暂停且非用户主动操作时（浏览器夺回控制权），也退出
...
function _onKey(e) {
    try {
        ...
        if (_inPlayer && isPlayerVisible) {
            // 播放器状态
            var now = Date.now();
            // 双击OK键(间隔<500ms)：切换播放速度
            if ((k === 13 || k === 32) && (now - _lastOkTime < 500)) {
                _lastOkTime = 0; // 重置，防止三击
                _cycleSpeed();
                return _killEvent(e);
            }
            // 单击OK键：播放/暂停（记录时间用于双击检测）
            if (k === 13 || k === 32) {
                _lastOkTime = now;
...
/* ============ popstate：拦截浏览器返回 ============ */
// 页面加载时 push 多个 state 做缓冲，防止 VIA 浏览器连续返回跳出 WebView
for (var _i = 0; _i < 5; _i++) {
    history.pushState({ page: "tv2", idx: _i }, "");
}

window.addEventListener("popstate", function(e) {
    // 浏览器返回触发 popstate，补回被消耗的 history，防止跳出页面
